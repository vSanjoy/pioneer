									...
									<div class="col-md-12 individual_section">
										...
										<div class="permission-content section_class">
											...
											@foreach($groupRow as $row)
												...
												<li>
													<div class="custom-control custom-checkbox">
														<input type="checkbox" name="role_page_ids[]" value="{{$row['role_page_id']}}" data-page="{{ $group }}" data-path="{{ $row['path'] }}" data-class="{{ $groupClass }}" data-listIndex="{{$listIndexClass}}" class="checkAll setPermission {{ $groupClass }} {{ $subClass }} selectDeselectAll" id="customCheck_{{$h}}_{{$listOrIndex}}">
