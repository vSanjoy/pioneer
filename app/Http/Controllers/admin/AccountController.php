            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    ...
                    if (Hash::check($request->current_password, $hashedPassword)) {
                        ...
                        if ($updatePassword) {
                            $this->generateToastMessage('success', trans('custom_admin.success_data_updated_successfully')." ".trans('custom_admin.success_for_security_reason_logged_out'), false);
                            Auth::guard('admin')->logout();
                            return redirect()->route($this->routePrefix.'.login');
                        } else {
                            $this->generateToastMessage('error', trans('custom_admin.error_took_place_while_updating'), false);
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    ...
                    if ($websiteSettings == null) {
                        ...
                        $saveData['logo_title']                 = $request->logo_title ?? null;
                        $saveData['logo_alt']                   = $request->logo_alt ?? null;
                        $saveData['footer_logo_title']          = $request->footer_logo_title ?? null;
                        $saveData['footer_logo_alt	']          = $request->footer_logo_alt	 ?? null;
                        ...
                        $updateData['logo_title']               = $request->logo_title ?? null;
                        $updateData['logo_alt']                 = $request->logo_alt ?? null;
                        $updateData['footer_logo_title']        = $request->footer_logo_title ?? null;
                        $updateData['footer_logo_alt']          = $request->footer_logo_alt	 ?? null;
