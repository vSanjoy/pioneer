...
# Author            :
# Created Date      :
# Page/Class name   : AreaReportController
...
use DataTables;
use Excel;
use App\Exports\DistributionAreaMultiSheetExport;


class AreaReportController extends Controller
{
    use GeneralMethods;
    public $controllerName  = 'AreaReport';
    public $management;
    public $modelName       = 'AreaReport';
    public $breadcrumb;
    public $routePrefix     = 'admin';
    public $pageRoute       = 'areaReport';
    public $listUrl         = 'areaReport.list';
    public $listRequestUrl  = 'areaReport.ajax-list-request';
    public $addUrl          = 'areaReport.add';
    public $editUrl         = 'areaReport.edit';
    public $statusUrl       = 'areaReport.change-status';
    public $deleteUrl       = 'areaReport.delete';
    public $viewFolderPath  = 'admin.areaReport';
    public $model           = 'AreaReport';
    ...
    */
    public function __construct() {
        ...
        $this->management  = trans('custom_admin.label_menu_area_report');
    ...
    */
    public function list(Request $request) {
        $data = [
            'pageTitle'     => trans('custom_admin.label_area_report_list'),
            'panelTitle'    => trans('custom_admin.label_area_report_list'),
            'pageType'      => 'LISTPAGE'
        ];
    ...
    */
    public function export(Request $request) {
        ...
        try {
            ...
            // Main query
            $data = $this->model->where(['status' => '1'])->whereNull(['deleted_at'])->orderBy('title', 'ASC');

            if ($filterByDistributionArea) {
                $data = $data->where('id', $distributionAreaId);
            }
            
            $data = $data->get();

            return Excel::download(new DistributionAreaMultiSheetExport($data), 'area_report_'.date('Y_m_d_H_i_s').'.xlsx');
        } catch (Exception $e) { 
