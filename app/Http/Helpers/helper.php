...
/*
    * Function name : cityDropdownMenu
    * Purpose       : This function is for city drop down menu
    * Author        :
    * Created Date  :
    * Modified Date : 
    * Input Params  : 
...
function cityDropdownMenu() {
    ...
    if (Session::get('cityId') && Session::get('stateId')) {
        $seasons    = Season::select('id','title')
                            ->where(['city_id' => Session::get('cityId'),'status' => '1'])
                            ->with([
                                'regionDetails' => function ($query) {
                                    $query->select('id', 'season_id', 'title')
                                        ->where('status', '1')
                                        ->whereNull(['deleted_at'])
                                        ->orderBy('sort', 'ASC')
                                        ->with([
                                            'playerTypeDetails' => function($subQuery) {
                                                $subQuery->select('id', 'season_id', 'region_id', 'title')
                                                        ->where('status', '1')
                                                        ->whereNull(['deleted_at'])
                                                        ->orderBy('sort', 'ASC');
                                            }
        ...
        if ($seasons->count()) {
            foreach ($seasons as $seasonKey => $seasonVal) {
                $dropDownMenu[$seasonVal->id]['id']     = $seasonVal->id;
                $dropDownMenu[$seasonVal->id]['title']  = $seasonVal->title;
                if ($seasonVal->regionDetails->count()) {
                    foreach ($seasonVal->regionDetails as $regionKey => $regionVal) {
                        $dropDownMenu[$seasonVal->id]['regions'][$regionKey]['id']      = $regionVal->id;
                        $dropDownMenu[$seasonVal->id]['regions'][$regionKey]['title']   = $regionVal->title;

                        if ($regionVal->playerTypeDetails->count()) {
                            foreach ($regionVal->playerTypeDetails as $playerTypeKey => $playerTypeVal) {
                                $dropDownMenu[$seasonVal->id]['regions'][$regionKey]['playerTypes'][$playerTypeKey]['id']     = $playerTypeVal->id;
                                $dropDownMenu[$seasonVal->id]['regions'][$regionKey]['playerTypes'][$playerTypeKey]['title']  = $playerTypeVal->title;
...
/*
    * Function name : cityDropdownMenuAccordingAssignment
    * Purpose       : This function is for city drop down menu according to the player assignment
    * Author        :
    * Created Date  :
    * Modified Date : 
    * Input Params  : 
    * Return Value  : drop down structure
*/
function cityDropdownMenuAccordingAssignment() {
    $dropDownMenu   = [];

    if (Session::get('cityId') && Session::get('stateId')) {
        $menuList = PlayerLeagueAssignment::select('id','city_id','season_id','region_id','player_type_id','competitive_level_id','title','sort')
                                            ->where(['city_id' => Session::get('cityId'), 'status' => '1'])
                                            ->whereNull(['deleted_at'])
                                            ->with([
                                                'cityDetails' => function($cityQuery) {
                                                    $cityQuery->select('id', 'title', 'slug');
                                                },
                                                'seasonDetails' => function($seasonQuery) {
                                                    $seasonQuery->select('id', 'title', 'slug');
                                                },
                                                'regionDetails' => function($regionQuery) {
                                                    $regionQuery->select('id', 'title', 'slug');
                                                },
                                                'playerTypeDetails' => function($playerTypeQuery) {
                                                    $playerTypeQuery->select('id', 'title', 'slug');
                                                },
                                                'competitiveLevelDetails' => function($competitiveLevelQuery) {
                                                    $competitiveLevelQuery->select('id', 'title', 'slug');
                                                },
                                            ])
                                            ->whereNull(['deleted_at'])
                                            ->orderBy('sort', 'ASC')
        ...
        if ($menuList->count()) {
            foreach ($menuList as $menuKey => $menuVal) {
                $seasonId           = $menuVal->season_id;
                $regionId           = $menuVal->region_id;
                $playerTypeId       = $menuVal->player_type_id;
                $competitiveLevelId = $menuVal->competitive_level_id;

                // Season
                $dropDownMenu[$seasonId]['id']          = $seasonId;
                $dropDownMenu[$seasonId]['title']       = $menuVal->seasonDetails->title;
                $dropDownMenu[$seasonId]['is_last_node']= 'N';

                $regionList = Region::select('id','title','slug')->where(['season_id' => $seasonId])->with(['playerTypeDetails'])->get();
                if ($regionList->count()) {
                    foreach ($regionList as $regionKey => $regionVal) {
                        if ($regionId == $regionVal->id) {
                            // Regions
                            $dropDownMenu[$seasonId]['regions'][$regionId]['id']            = $regionId;
                            $dropDownMenu[$seasonId]['regions'][$regionId]['title']         = $regionVal->title;
                            $dropDownMenu[$seasonId]['regions'][$regionId]['is_last_node']  = 'N';

                            // Player types
                            if ($playerTypeId && $regionVal->playerTypeDetails->count()) {
                                foreach ($regionVal->playerTypeDetails as $playerTypeKey => $playerTypeVal) {
                                    if ($playerTypeId == $playerTypeVal->id) {
                                        $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['id']             = $playerTypeId;
                                        $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['title']          = $playerTypeVal->title;
                                        $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['is_last_node']   = 'N';

                                        // Start :: Completitive levels

                                        // With Assignment Name First > Silver / Bronze...
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$menuVal->title]['id'] = customEncryptionDecryption($menuVal->id);
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$menuVal->title]['rawid'] = $menuVal->id;
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$menuVal->title]['title'] = $menuVal->competitiveLevelDetails->title;
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$menuVal->title]['is_last_node'] = 'Y';


                                        // // With Silver / Bronze > Assignment Name Last
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['id'] = $menuVal->id;
                                        // // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['rawid'] = $menuVal->id;
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['title'] = $menuVal->competitiveLevelDetails->title;
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['is_last_node'] = 'N';
                                        // //  +
                                        // $assignmentPlayerTypeTitle = strtolower($menuVal->title);
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$assignmentPlayerTypeTitle]['id'] = customEncryptionDecryption($menuVal->id);
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$assignmentPlayerTypeTitle]['rawid'] = $menuVal->id;
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$assignmentPlayerTypeTitle]['title'] = $menuVal->title;
                                        // $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['assignment_types'][$assignmentPlayerTypeTitle]['is_last_node'] = 'Y';
                                        

                                        // Without Assignment Name
                                        $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['id'] = customEncryptionDecryption($menuVal->id);
                                        $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['rawid'] = $menuVal->id;
                                        $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['title'] = $menuVal->competitiveLevelDetails->title;

                                        if ($menuVal->title == '') {
                                            $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['is_last_node']= 'Y';
                                        } else {
                                            $dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels'][$competitiveLevelId]['is_last_node']= 'N';
                                        ...
                                        // Sorting according to competititve levels id
                                        // asort($dropDownMenu[$seasonId]['regions'][$regionId]['player_types'][$playerTypeId]['competitive_levels']);

                                        // End :: Completitive levels
                                    }
                                }
                            } else {
                                $dropDownMenu[$seasonId]['regions'][$regionId]['league_divisions'][$competitiveLevelId]['id']           = customEncryptionDecryption($menuVal->id);
                                $dropDownMenu[$seasonId]['regions'][$regionId]['league_divisions'][$competitiveLevelId]['rawid']        = $menuVal->id;
                                $dropDownMenu[$seasonId]['regions'][$regionId]['league_divisions'][$competitiveLevelId]['title']        = $menuVal->competitiveLevelDetails->title;
                                $dropDownMenu[$seasonId]['regions'][$regionId]['league_divisions'][$competitiveLevelId]['is_last_node'] = 'Y';
                            }
                        }
                    }
                }
...
/*
    * Function name : getUserMatchDetails
    * Purpose       : This function is to get match details from Score per User
    * Author        :
    * Created Date  :
    * Modified Date : 
    * Input Params  : 
    * Return Value  : 
*/
function getUserMatchDetails($userId = null, $leagueId = null) {  
	$totalMatchNumberPerUser        = Score::where('winner_id', $userId)
                                            ->orwhere('loser_id', $userId)
                                            ->count();
	$totalWinningMatchNumberPerUser = Score::where('winner_id', $userId)->count();

	$totalLooseMatchNumberPerUser   = Score::where('loser_id', $userId)->count();
									  
	$totalMatchNumberPerLegue       = Score::where('player_league_assignment_id', $leagueId)
                                            ->where(function($query) use ($userId) {
                                                $query->where('winner_id', $userId)
                                                        ->orwhere('loser_id', $userId);
                                            })->count();
						
	$totalWinningMatchNumberPerLegue= Score::where('player_league_assignment_id', $leagueId)
                                            ->where(function($query) use ($userId) {
                                                $query->where('winner_id', $userId);
                                            })->count();
	
	$totalLooseMatchNumberPerLegue  = Score::where('player_league_assignment_id', $leagueId)
                                                ->where(function($query) use ($userId) {
                                                    $query->where('loser_id', $userId);
                                                })->count();

	$userMatchDetails = $totalMatchNumberPerUser."|".$totalWinningMatchNumberPerUser."|".$totalLooseMatchNumberPerUser."|".$totalMatchNumberPerLegue."|".$totalWinningMatchNumberPerLegue."|".$totalLooseMatchNumberPerLegue;
	
    return $userMatchDetails;
}

/*
    * Function name : getTeamMatchDetails
    * Purpose       : This function is to get match details from Score per Team
    * Author        :
    * Created Date  :
    * Modified Date : 
    * Input Params  : 
    * Return Value  : 
*/
function getTeamMatchDetails($teamId = null, $leagueId = null) {
	
	$totalMatchNumberPerLegue       = Score::where('player_league_assignment_id', $leagueId)
                                            ->where(function($query) use ($teamId) {
                                                $query->where('winner_team_id', $teamId)
                                                        ->orwhere('loser_team_id', $teamId);
                                            })->count();
	...
	$totalLooseMatchNumberPerLegue = Score::where('player_league_assignment_id', $leagueId)
                                                ->where(function($query) use ($teamId) {
						                            $query->where('loser_team_id', $teamId);
						                        })->count();

	$teamMatchDetails = $totalMatchNumberPerLegue."|".$totalWinningMatchNumberPerLegue."|".$totalLooseMatchNumberPerLegue;
	
    return $teamMatchDetails;
}

/*
    * Function name : getSinglesOverallRecord
    * Purpose       : This function is to get overall record
    * Author        :
    * Created Date  :
    * Modified Date : 
    * Input Params  : 
    * Return Value  : 
*/
function getSinglesOverallRecord($playerId = null) {
	...
	$overallRecord = $overallWin."|".$overallLose;
	
    return $overallRecord;
}

/*
    * Function name : getTeamOverallRecord
    * Purpose       : This function is to get overall record
    * Author        :
    * Created Date  :
    * Modified Date : 
    ...
    * Return Value  : 
*/
function getTeamOverallRecord($teamId = null) {
	$overallWin     = Score::where('winner_team_id', $teamId)->count();
    $overallLose    = Score::where('loser_team_id', $teamId)->count();
...
function isAccessBlocked() {
    ...
    $accessWillEnds     = date('2022-04-02 00:00:00');
